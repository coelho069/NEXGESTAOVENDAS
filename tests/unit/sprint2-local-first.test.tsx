...
describe("isolamento do storage por sessão", () => {
  it("usa bancos distintos por usuário, limpa carrinho e preserva outbox pendente", async () => {
    ...
    expect(getPdvLocalDbName(userA)).not.toBe(getPdvLocalDbName(userB));

    const dbA = getPdvLocalDbForUser(userA);
    await dbA.open();
    dbs.push(dbA);
    await seedStock(dbA);
    await closeSale(dbA, saleInput());
    useCartStore.getState().setLines([sampleLine]);
    localStorage.setItem(CART_PERSIST_KEY, JSON.stringify({ lines: [sampleLine] }));

    await clearClientSessionStorage(userA);

    expect(localStorage.getItem(CART_PERSIST_KEY)).toBeNull();
    const reopenedA = createPdvLocalDb(getPdvLocalDbName(userA));
    await reopenedA.open();
    expect(await reopenedA.outbox.count()).toBe(1);
    reopenedA.close();
    await deletePdvLocalDb(getPdvLocalDbName(userA));

    const dbB = getPdvLocalDbForUser(userB);
    await dbB.open();
    dbs.push(dbB);
    ...
    const reopenedB = createPdvLocalDb(getPdvLocalDbName(userB));
    await reopenedB.open();
    expect(await reopenedB.inventoryBalances.count()).toBe(0);
    reopenedB.close();
    await deletePdvLocalDb(getPdvLocalDbName(userB));
  });
});
...
describe("reconcileSale e pullChanges", () => {
  ...
  it("consome páginas de vendas sem avançar o cursor além da página truncada", async () => {
    ...
    const lastSale = firstPage.at(-1)!;
    const fetchFn = vi.fn(async (input: RequestInfo | URL) => {
      const params = new URL(String(input), "http://localhost").searchParams;
      if (fetchFn.mock.calls.length === 1) {
        expect(params.get("sales_after_id")).toBeNull();
        return new Response(
          JSON.stringify({
            serverTime: "2026-09-01T12:04:00.000Z",
            has_more: true,
            next_cursor: {
              since: null,
              sales_after_updated_at: lastSale.updated_at,
              sales_after_id: lastSale.id,
            },
            inventory: [],
            sales: firstPage,
          }),
      ...
      expect(params.get("sales_after_id")).toBe(lastSale.id);
      return new Response(
        JSON.stringify({
    ...
    await pullChanges({ db, storeId: STORE_ID, fetchFn });

    expect(fetchFn).toHaveBeenCalledTimes(2);
    expect(await db.meta.get(`lastPullAt:${STORE_ID}`)).toEqual({
      key: `lastPullAt:${STORE_ID}`,
