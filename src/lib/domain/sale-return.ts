...
export const CASH_SESSION_CLOSED_FOR_REFUND = "cash_session_closed_for_refund";

export const CASH_SESSION_CLOSED_FOR_REFUND_MESSAGE =
  "A sessão de caixa original desta venda está fechada ou não confere com o terminal. Use um ajuste de caixa explícito — o estorno em dinheiro não pode ir para o caixa aberto de hoje.";

export type CashRefundSessionInput = {
  paymentMethod: string;
  saleCashSessionId: string | null | undefined;
  requestedCashSessionId: string | null | undefined;
  originalSession: {
    status: string;
    terminalId: string;
    storeId: string;
...
export function evaluateCashRefundSession(
  input: CashRefundSessionInput
): { ok: true; cashSessionId: string | undefined } | { ok: false; error: string; message: string } {
  if (input.paymentMethod !== "cash") {
    return { ok: true, cashSessionId: undefined };
  }

  if (!input.saleCashSessionId || !input.originalSession) {
    return {
      ok: false,
      error: CASH_SESSION_CLOSED_FOR_REFUND,
      message: CASH_SESSION_CLOSED_FOR_REFUND_MESSAGE,
    };
  }

  if (
    input.requestedCashSessionId &&
    input.requestedCashSessionId !== input.saleCashSessionId
  ) {
    return {
      ok: false,
      error: CASH_SESSION_CLOSED_FOR_REFUND,
  ...
  if (
    input.originalSession.status !== "open" ||
    input.originalSession.terminalId !== input.requestTerminalId ||
    input.originalSession.storeId !== input.requestStoreId
  ) {
    return {
      ok: false,
      error: CASH_SESSION_CLOSED_FOR_REFUND,
  ...
  return { ok: true, cashSessionId: input.saleCashSessionId };
}
