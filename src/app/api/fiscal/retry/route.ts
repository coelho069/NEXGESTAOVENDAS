...
import { fiscalRetryInputSchema } from "@/lib/validation/schemas";
...
export async function POST(request: Request) {
  ...
  const parsed = fiscalRetryInputSchema.safeParse(json);
  if (!parsed.success) {
  ...
  if (!getFiscalProviderConfig().configured) {
    return NextResponse.json({
      ...parsed.data,
  ...
  const { data, error } = await supabase.rpc("retry_fiscal_operation", {
