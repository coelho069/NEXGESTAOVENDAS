...
export function CartPanel({
  ...
  return (
    ...
    >
      ...
      <p className="mt-0.5 text-xs text-slate-500">+/- altera a linha selecionada</p>
          ...
          lines.map((line) => {
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-3 transition ${selected ? "border-emerald-500 bg-emerald-50" : "border-slate-100 hover:border-slate-200"}`}
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      {line.sku} · <span className="tabular-nums">{formatBRL(line.unitPrice)}</span>
                    </div>
                    <StockLabel
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 outline-none transition hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-indigo-200"
                      onClick={(event) => {
                        event.stopPropagation();
                    ...
                    <input
                      ...
                      className="h-8 w-14 rounded-lg border border-slate-200 text-center tabular-nums outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                      value={line.quantity}
                      onChange={(event) => {
                  ...
                  <div className="font-semibold tabular-nums text-slate-900">{formatBRL(lineTotal(line))}</div>
                </div>
              </div>
            );
          })
