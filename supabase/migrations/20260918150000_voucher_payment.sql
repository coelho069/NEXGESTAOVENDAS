-- PDV direct checkout: voucher ("Vale") mirrors cash/pix_manual (confirmed/synced, no provider_ref).
-- sale_cash ledger remains cash-only; voucher skips drawer credit like pix_manual.
...
BEGIN
  ...
  IF COALESCE(jsonb_typeof((p_payload->'payments')->0) <> 'object', true)
    ...
    OR (p_payload->'payments'->0->>'method') NOT IN ('cash', 'pix_manual', 'voucher')
...
-- process_sale_core: allow cash, pix_manual, and voucher (provider rails stay outside this RPC).
