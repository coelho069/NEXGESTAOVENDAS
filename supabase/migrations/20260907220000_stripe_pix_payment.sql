-- Stripe PIX payments (testmode): pending QR ≠ confirmed sale.
-- Cash RPCs, cash sessions and cash movements are untouched.
-- PIX refunds stay payment_refund_status=pending_external until webhook/reconcile.
...
CREATE TABLE IF NOT EXISTS public.pix_payment_intents (
  ...
  CONSTRAINT pix_payment_intents_store_scope_fk
    FOREIGN KEY (store_id, org_id)
    REFERENCES public.stores (id, org_id)
);

CREATE UNIQUE INDEX IF NOT EXISTS pix_payment_intents_mutation_key
  ON public.pix_payment_intents (org_id, store_id, client_mutation_id);

CREATE UNIQUE INDEX IF NOT EXISTS pix_payment_intents_provider_ref_key
  ON public.pix_payment_intents (provider_ref);

CREATE TABLE IF NOT EXISTS public.pix_provider_events (
...
CREATE INDEX IF NOT EXISTS pix_provider_events_provider_ref_idx
  ON public.pix_provider_events (provider_ref, created_at DESC);

ALTER TABLE public.pix_payment_intents ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pix_provider_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS pix_payment_intents_deny ON public.pix_payment_intents;
CREATE POLICY pix_payment_intents_deny
  ON public.pix_payment_intents
  FOR ALL
  TO authenticated
  USING (false)
  WITH CHECK (false);

DROP POLICY IF EXISTS pix_provider_events_deny ON public.pix_provider_events;
CREATE POLICY pix_provider_events_deny
  ON public.pix_provider_events
  FOR ALL
  TO authenticated
  USING (false)
  WITH CHECK (false);

REVOKE ALL ON TABLE public.pix_payment_intents FROM PUBLIC;
REVOKE ALL ON TABLE public.pix_payment_intents FROM anon;
REVOKE ALL ON TABLE public.pix_payment_intents FROM authenticated;
REVOKE ALL ON TABLE public.pix_provider_events FROM PUBLIC;
REVOKE ALL ON TABLE public.pix_provider_events FROM anon;
REVOKE ALL ON TABLE public.pix_provider_events FROM authenticated;
GRANT SELECT ON TABLE public.pix_payment_intents TO service_role;
GRANT SELECT ON TABLE public.pix_provider_events TO service_role;

CREATE OR REPLACE FUNCTION public.register_pix_payment_intent(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_status text;
  v_org_id uuid;
  v_existing public.pix_payment_intents%ROWTYPE;
  v_row public.pix_payment_intents%ROWTYPE;
BEGIN
  ...
  EXCEPTION
    WHEN invalid_text_representation OR numeric_value_out_of_range THEN
      RAISE EXCEPTION 'invalid_pix_payment_intent' USING ERRCODE = '22023';
  END;
  ...
  IF FOUND THEN
    ...
    THEN
      RAISE EXCEPTION 'pix_payment_intent_mismatch' USING ERRCODE = '22023';
    END IF;
  ...
  ELSE
    INSERT INTO public.pix_payment_intents (
      org_id,
      store_id,
      client_mutation_id,
      provider_ref,
...
CREATE OR REPLACE FUNCTION public.apply_pix_provider_status(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
BEGIN
  ...
  IF v_provider_ref IS NULL
    OR v_provider_ref !~ '^pi_[A-Za-z0-9]+$'
    OR v_status NOT IN ('pending', 'authorized', 'captured', 'failed', 'unknown', 'cancelled', 'refunded')
  THEN
    RAISE EXCEPTION 'invalid_pix_provider_status' USING ERRCODE = '22023';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'pix_payment_intent_not_found' USING ERRCODE = '42501';
  END IF;
...
CREATE OR REPLACE FUNCTION public.apply_pix_provider_event(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
DECLARE
  ...
  v_provider_ref text;
  v_status text;
  v_intent public.pix_payment_intents%ROWTYPE;
  v_existing public.pix_provider_events%ROWTYPE;
BEGIN
  ...
  IF v_event_id IS NULL OR v_event_type IS NULL OR v_provider_ref IS NULL THEN
    RAISE EXCEPTION 'invalid_pix_provider_event' USING ERRCODE = '22023';
  END IF;

  SELECT *
  INTO v_existing
  FROM public.pix_provider_events
  ...
  IF FOUND THEN
    ...
    INTO v_intent
    FROM public.pix_payment_intents
    WHERE provider_ref = v_existing.provider_ref;
    RETURN jsonb_build_object(
      'event_id', v_existing.event_id,
      'provider_ref', v_existing.provider_ref,
      'status', v_existing.status,
      'store_id', COALESCE(v_existing.store_id, v_intent.store_id),
      'client_mutation_id', v_intent.client_mutation_id,
  ...
  INSERT INTO public.pix_provider_events (
    event_id,
    org_id,
    store_id,
    provider_ref,
...
CREATE OR REPLACE FUNCTION public.process_pix_sale(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
BEGIN
  ...
  EXCEPTION
    WHEN invalid_text_representation OR numeric_value_out_of_range THEN
      RAISE EXCEPTION 'invalid_pix_sale' USING ERRCODE = '22023';
  END;
  ...
  IF v_intent.status <> 'captured' THEN
    RAISE EXCEPTION 'pix_payment_not_captured' USING ERRCODE = '22023';
  END IF;
...
CREATE OR REPLACE FUNCTION public.complete_pix_refund(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
BEGIN
  ...
  IF v_provider_ref IS NULL OR v_event_id IS NULL THEN
    RAISE EXCEPTION 'invalid_pix_refund' USING ERRCODE = '22023';
  END IF;

  -- Never insert refund_cash. PIX refunds stay pending_external until a
  -- succeeded refund/charge.refunded event arrives.
  PERFORM public.apply_pix_provider_event(jsonb_build_object(
    'event_id', v_event_id,
...
REVOKE ALL ON FUNCTION public.register_pix_payment_intent(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.register_pix_payment_intent(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.register_pix_payment_intent(jsonb) TO authenticated;

REVOKE ALL ON FUNCTION public.process_pix_sale(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.process_pix_sale(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.process_pix_sale(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.process_pix_sale(jsonb) TO service_role;

REVOKE ALL ON FUNCTION public.apply_pix_provider_status(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.apply_pix_provider_status(jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.apply_pix_provider_status(jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.apply_pix_provider_status(jsonb) TO service_role;

REVOKE ALL ON FUNCTION public.apply_pix_provider_event(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.apply_pix_provider_event(jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.apply_pix_provider_event(jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.apply_pix_provider_event(jsonb) TO service_role;

CREATE OR REPLACE FUNCTION public.get_pix_payment_intent(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
REVOKE ALL ON FUNCTION public.get_pix_payment_intent(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_pix_payment_intent(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_pix_payment_intent(jsonb) TO authenticated;

REVOKE ALL ON FUNCTION public.complete_pix_refund(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.complete_pix_refund(jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.complete_pix_refund(jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.complete_pix_refund(jsonb) TO service_role;
