-- Mercado Pago Orders PIX: reuse pix_payment_intents / pix_provider_events with ORD* provider_ref.
-- Stripe pi_* refs remain valid. No new tables; same register / apply / process_pix_sale path.
...
ALTER TABLE public.pix_payment_intents
  DROP CONSTRAINT IF EXISTS pix_payment_intents_provider_ref_check;

ALTER TABLE public.pix_payment_intents
  ADD CONSTRAINT pix_payment_intents_provider_ref_check
  CHECK (public.is_valid_pix_provider_ref(provider_ref));
...
BEGIN
  ...
  IF v_store_id IS NULL
    ...
    OR NOT public.is_valid_pix_provider_ref(v_provider_ref)
    OR v_amount IS NULL
    OR v_amount <= 0
