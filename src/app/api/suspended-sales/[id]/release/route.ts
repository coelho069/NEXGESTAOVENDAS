...
import { releaseSuspendedSaleInputSchema } from "@/lib/validation/schemas";
...
export async function POST(
  ...
  const parsed = releaseSuspendedSaleInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { data, error } = await supabase.rpc("release_suspended_sale", {
    p_payload: {
  ...
  if (error) {
    ...
    if (message.includes("conflict") || message.includes("completed")) {
