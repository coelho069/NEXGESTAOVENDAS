...
export async function recordCapturedCardSale(
  db: PdvLocalDatabase,
  input: CloseSaleInput & { serverSaleId: string; providerReference: string }
): Promise<CloseSaleResult> {
  assertNoSecrets(input, "recordCapturedCardSale");
  if (input.payments.length !== 1 || input.payments[0]?.method !== "card") {
    throw new Error("recordCapturedCardSale exige exatamente um pagamento card");
  }
  ...
  if (money(total).lt(0) || input.payments[0]?.amount !== total) {
    throw new Error("Pagamento card deve corresponder ao total da venda");
  }
