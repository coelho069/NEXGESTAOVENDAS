...
import { mercadopagoPixPaymentInputSchema, storeIdSchema } from "@/lib/validation/schemas";
...
import {
  ...
  getMercadoPagoPixAdapterHealth,
...
export async function GET(request: Request) {
  const obs = createRequestObservability(request, "payments.mercadopago.pix.health");
  ...
  const health = await getMercadoPagoPixAdapterHealth();
  ...
  return obs.withHeaders(
    NextResponse.json({
      ...
      message: health.configured
        ? health.message
        : health.reason === "mercadopago_checkout_hold"
          ? health.message
          : "Adapter Mercado Pago PIX não configurado.",
    })
  );
}

export async function POST(request: Request) {
  const obs = createRequestObservability(request, "payments.mercadopago.pix");
  let supabase: Awaited<ReturnType<typeof createClient>>;
  try {
    supabase = await createClient();
  } catch {
  ...
  const rate = consumeRateLimit({
    key: clientRateLimitKey(request, "payments-mercadopago-pix", user.id),
    limit: 30,
    windowMs: 60_000,
  });
  ...
  const parsed = mercadopagoPixPaymentInputSchema.safeParse(json);
  if (!parsed.success) {
    return obs.withHeaders(
  ...
  const result = await executeMercadoPagoPixPayment(supabase, parsed.data, user.id);
